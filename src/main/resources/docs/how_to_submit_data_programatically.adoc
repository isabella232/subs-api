...
is up to you. You could create a team for just yourself, everyone in a lab, or one for all collaborators on At this stage of development, the API doesn't restrict what team you use.
...
TODO document endpoints that show valid statuses for a submission

You can then change the submission status to 'Submitted', using either a `PUT` or `PATCH` request:
