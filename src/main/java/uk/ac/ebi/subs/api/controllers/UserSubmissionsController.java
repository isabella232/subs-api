...
public class UserSubmissionsController {
    ...
    public PagedModel<EntityModel<SubmissionWithStatus>> getUserSubmissions(Pageable pageable) {
        List<String> userTeamNames = userTeamService.userTeamNames();

        Page<Submission> page = submissionRepository.findByTeamNameInOrderByLastModifiedDateDesc(userTeamNames, pageable);
    ...
    public EntityModel<Map<String, Integer>> getUserSubmissionStatusSummary() {
        List<String> userTeamNames = userTeamService.userTeamNames();
        ...
        return new EntityModel<>(statusCounts,self);
    }
}
